...
int aodv_db_capt_rreq (uint8_t dhost_ether[ETH_ALEN], uint8_t shost_ether[ETH_ALEN],
	...
	aodv_db_wlock();
	int result = aodv_db_rt_capt_rreq(dhost_ether, shost_ether, shost_prev_hop, output_iface, rreq_seq, hop_count, path_weight, timestamp);
	aodv_db_unlock();
	return result;
}
...
int aodv_db_capt_rrep (uint8_t dhost_ether[ETH_ALEN], uint8_t dhost_next_hop[ETH_ALEN],
	...
	aodv_db_wlock();
	int result =  aodv_db_rt_capt_rrep(dhost_ether, dhost_next_hop, output_iface, rrep_seq, hop_count, path_weight, timestamp);
	aodv_db_unlock();
	return result;
}
