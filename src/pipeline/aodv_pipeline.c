...
int aodv_handle_rreq(dessert_msg_t* msg, size_t len, dessert_msg_proc_t *proc, dessert_meshif_t *iface, dessert_frameid_t id) {
	...
	int x = aodv_db_capt_rreq(l25h->ether_dhost, l25h->ether_shost, msg->l2h.ether_shost, iface, rreq_msg->seq_num_src, rreq_msg->path_weight, &ts);
	if(x == -1) {
	...
	if (memcmp(dessert_l25_defsrc, l25h->ether_dhost, ETH_ALEN) != 0) { // RREQ not for me
		...
		if(!f) {
			...
			pthread_rwlock_unlock(&pp_rwlock);
			dessert_msg_t* rrep_msg = _create_rrep(l25h->ether_dhost, l25h->ether_shost, msg->l2h.ether_shost, seq_num_copy, AODV_FLAGS_RREP_A);
		...
		int y = aodv_db_capt_rrep(l25h->ether_shost, prev_hop, iface, rreq_msg->seq_num_src, rreq_msg->hop_count, rreq_msg->path_weight, &ts);
		if (y == TRUE) {
...
int aodv_handle_rrep(dessert_msg_t* msg, size_t len, dessert_msg_proc_t *proc, dessert_meshif_t *iface, dessert_frameid_t id){
	...
	int x = aodv_db_capt_rrep(l25h->ether_shost, msg->l2h.ether_shost, iface, rrep_msg->seq_num_dest, rrep_msg->hop_count, rrep_msg->path_weight, &ts);
	if(x == -1) {
		dessert_crit("aodv_db_capt_rreq returns error");
		return DESSERT_MSG_DROP;
	}
	...
	if (memcmp(dessert_l25_defsrc, l25h->ether_dhost, ETH_ALEN) != 0) {
		...
		dessert_debug("got RREP from " MAC " seq=%d path_weight=%d -> aodv_send_packets_from_buffer", EXPLODE_ARRAY6(l25h->ether_shost), aodv_msg_rrep->seq_num_dest, rrep_msg->path_weight);
...
int aodv_sys2rp (dessert_msg_t *msg, size_t len, dessert_msg_proc_t *proc, dessert_sysif_t *sysif, dessert_frameid_t id) {
	...
	if (memcmp(l25h->ether_dhost, ether_broadcast, ETH_ALEN) == 0) {
		...
		if (aodv_db_getroute2dest(l25h->ether_dhost, dhost_next_hop, &output_iface, &ts) == TRUE) {
			...
			aodv_send_rreq(l25h->ether_dhost, &ts, TTL_START, 0); // create and send RREQ
