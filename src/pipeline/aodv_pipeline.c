...
dessert_msg_t* _create_rrep(uint8_t route_dest[ETH_ALEN], uint8_t route_source[ETH_ALEN],
		uint8_t rrep_next_hop[ETH_ALEN], uint32_t rrep_seq, uint8_t flags, uint8_t initial_path_weight) {
	dessert_msg_t* msg;
	dessert_ext_t* ext;
	dessert_msg_new(&msg);
...
void aodv_send_rreq(uint8_t dhost_ether[ETH_ALEN], struct timeval* ts, uint8_t ttl, uint8_t initial_path_weight) {
...
int aodv_handle_rreq(dessert_msg_t* msg, size_t len, dessert_msg_proc_t *proc, dessert_meshif_t *iface, dessert_frameid_t id) {
	...
	int x = aodv_db_capt_rreq(l25h->ether_dhost, l25h->ether_shost, msg->l2h.ether_shost, iface, rreq_msg->rreq_seq, rreq_msg->hop_count, rreq_msg->path_weight, &ts);
	if(x != TRUE) {
...
int aodv_handle_rrep(dessert_msg_t* msg, size_t len, dessert_msg_proc_t *proc, dessert_meshif_t *iface, dessert_frameid_t id){
	...
	int x = aodv_db_capt_rrep(l25h->ether_shost, msg->l2h.ether_shost, iface, rrep_msg->rrep_seq, rrep_msg->hop_count, rrep_msg->path_weight, &ts);
	if(x != TRUE) {
...
int aodv_sys2rp (dessert_msg_t *msg, size_t len, dessert_msg_proc_t *proc, dessert_sysif_t *sysif, dessert_frameid_t id) {
	...
	if (memcmp(l25h->ether_dhost, ether_broadcast, ETH_ALEN) == 0) {
		...
		if(a == TRUE) {
			...
			aodv_send_rreq(l25h->ether_dhost, &ts, TTL_START, 0); // create and send RREQ - without initial path_weight
